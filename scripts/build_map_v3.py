...
DISTRICTS = {
    ...
    "yuen long": (22.4438, 114.0220),
    "sha tin": (22.3805, 114.1898),
    "ma on shan": (22.4250, 114.2310),
    "tsing yi": (22.3590, 114.1080),
...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* Filter panel — visible by default; mobile collapses it */
  .filter-panel { display: flex; flex-direction: column; gap: 9px; }
  ...
  @media (max-width: 800px) {
    ...
    #sidebar {
      ...
      height: 92vh;
      height: 92dvh;
      transform: translate3d(0, calc(100% - 104px), 0);
      -webkit-transform: translate3d(0, calc(100% - 104px), 0);
      transition: transform 0.34s cubic-bezier(0.32, 0.72, 0, 1);
      -webkit-transition: -webkit-transform 0.34s cubic-bezier(0.32, 0.72, 0, 1);
    ...
    .filter-toggle[aria-expanded="true"] { color: var(--text); border-color: var(--accent); }
    .filter-toggle[aria-expanded="true"]::after { transform: rotate(180deg); }

    /* Mobile: keep display:flex (so transitions work), collapse via max-height */
    .filter-panel {
      max-height: 0;
      overflow: hidden;
...
<div id="app">
  <aside id="sidebar">
    ...
    <div class="controls">
      ...
      <button id="filter-toggle" class="filter-toggle" type="button" aria-expanded="false" data-i18n="filtersBtn">Filters</button>
...
// Welcome modal logic: open on first ever visit; for returning visitors on mobile, just expand the drawer
let hasSeenWelcome = false;
try { hasSeenWelcome = !!localStorage.getItem("hkv-seen-welcome-v4"); } catch (_) {}
if (!hasSeenWelcome) {
  openModal();
} else if (isMobile()) {
  // Returning mobile visitor: surface the list straight away
  expandSheet();
}
...
function applyDrawerState(expanded) {
  if (expanded) {
    ...
    sidebar.style.transform = "translate3d(0, calc(100% - 104px), 0)";
    sidebar.style.webkitTransform = "translate3d(0, calc(100% - 104px), 0)";
  }
}
