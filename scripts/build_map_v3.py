...
<style>
  /* ─── Base / palette (nicked from chitintim.github.io) ─── */
  :root {
    --bg: #05050a;
    --bg-lighter: #0a0a14;
    ...
    /* Category accents */
    --catA: #ffe66d;   /* Chinese 齋 — warm yellow */
    --catB: #4ecdc4;   /* Modern plant-based — teal */
    --catC: #ff6b6b;   /* Mainstream — coral */
    --catD: #a855f7;   /* Indian/ME/Med — purple */
    ...
    --glow-coral: rgba(255, 107, 107, 0.12);
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* ─── Ambient orbs (a single subtle one — not the full chaos) ─── */
  .orb {
    position: fixed;
    border-radius: 50%;
    filter: blur(80px);
  ...
  .controls input[type=search] { padding: 9px 12px; border: 1px solid var(--card-border-strong); border-radius: 10px; font-size: 14px; width: 100%; background: rgba(255,255,255,0.04); color: var(--text); }
  .controls input[type=search]::placeholder { color: var(--text-mute); }
  .controls input[type=search]:focus { outline: 2px solid var(--catB); outline-offset: -1px; border-color: var(--catB); background: rgba(255,255,255,0.06); }
  ...
  select { padding: 6px 10px; border: 1px solid var(--card-border-strong); border-radius: 10px; font-size: 12.5px; background: rgba(255,255,255,0.04); color: var(--text); min-height: 32px; }
  select:focus { outline: 2px solid var(--catB); outline-offset: -1px; }
  ...
  .item .score-badge.D { background: var(--tierD); color: #2a0606; } .item .score-badge.dash { background: rgba(255,255,255,0.08); color: var(--text-mute); font-size: 16px; }
  ...
  .item .pill { background: rgba(255,255,255,0.06); padding: 2px 8px; border-radius: 9px; }
  .item .pill.michelin { background: rgba(255, 230, 109, 0.14); color: var(--catA); }
  .item .pill.closed { background: rgba(255, 107, 107, 0.16); color: var(--tierD); }
  ...
  .popup .score-badge.D { background: var(--tierD); color: #2a0606; } .popup .score-badge.dash { background: rgba(255,255,255,0.08); color: var(--text-mute); }
  .popup h3 { margin: 0 0 2px; font-size: 15px; color: var(--text); }
  .popup .zh { color: var(--text-mute); font-size: 12px; }
  .popup .cat-tag { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 11px; color: #062520; margin-top: 4px; font-weight: 600; }
  ...
  .popup .rating { background: rgba(255,255,255,0.06); padding: 2px 8px; border-radius: 8px; font-size: 11px; color: var(--text); }
  .popup .rating.michelin { background: rgba(255, 230, 109, 0.16); color: var(--catA); }
  .popup .flag { margin-top: 8px; padding: 6px 10px; background: rgba(255, 230, 109, 0.1); border-left: 3px solid var(--catA); font-size: 11.5px; border-radius: 4px; color: var(--text); }
  .popup .flag.warn { background: rgba(255, 107, 107, 0.1); border-color: var(--tierD); }
  .popup a { color: var(--catB); text-decoration: none; }
  ...
  /* ─── Map controls / legend ──────────────────────── */
  .legend { font-size: 11px; line-height: 1.55; background: rgba(8, 8, 14, 0.86); color: var(--text); backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px); border: 1px solid var(--card-border-strong) !important; }
  .legend .row { display: flex; align-items: center; gap: 6px; }
  .legend .swatch { width: 10px; height: 10px; border-radius: 50%; border: 1.5px solid var(--bg-lighter); box-shadow: 0 0 0 1px rgba(255,255,255,0.2); }
  .legend strong { color: var(--text); }
  .legend small { color: var(--text-mute); }
  .leaflet-control-attribution { font-size: 10px; background: rgba(8,8,14,0.7) !important; color: var(--text-mute) !important; }
  .leaflet-control-attribution a { color: var(--text-dim) !important; }
  .leaflet-control-zoom a { background: var(--bg-lighter) !important; color: var(--text) !important; border-color: var(--card-border-strong) !important; }
  ...
  .modal::before {
    ...
    background: linear-gradient(135deg, var(--glow-teal), transparent 50%, var(--glow-purple));
    opacity: 0.4; pointer-events: none; z-index: 0;
  }
  @keyframes slide-up { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
  .modal-header { padding: 26px 28px 14px; position: relative; z-index: 1; }
  .modal-header h2 { margin: 0 0 6px; font-size: 24px; font-weight: 800; letter-spacing: -0.02em; background: linear-gradient(135deg, var(--catB), var(--catA), var(--catC)); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; }
  .modal-header h2 .leaf { -webkit-text-fill-color: initial; }
  .modal-header .sub { color: var(--text-dim); font-size: 14px; line-height: 1.5; margin: 0; }
  .modal-body { padding: 0 28px 16px; font-size: 13.5px; line-height: 1.6; color: var(--text); position: relative; z-index: 1; }
  .modal-body h3 { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; color: var(--catB); margin: 18px 0 8px; }
  .modal-body p { margin: 0 0 10px; }
  .modal-body ul { margin: 0 0 10px; padding-left: 18px; }
  .modal-body li { margin-bottom: 5px; color: var(--text-dim); }
  .modal-body li strong { color: var(--text); }
  .modal-body code { background: rgba(78, 205, 196, 0.12); padding: 1px 6px; border-radius: 4px; font-family: 'SF Mono', ui-monospace, Menlo, monospace; font-size: 12px; color: var(--catB); }
  ...
  .stat-pill:hover { transform: translateY(-2px); border-color: var(--card-border-strong); }
  .stat-pill .num { font-family: 'SF Mono', ui-monospace, Menlo, monospace; font-size: 22px; font-weight: 800; line-height: 1; color: var(--catB); }
  .stat-pill .lbl { font-size: 10px; color: var(--text-mute); margin-top: 4px; text-transform: uppercase; letter-spacing: 0.06em; }
  .modal-footer { padding: 14px 28px 24px; display: flex; gap: 12px; justify-content: flex-end; align-items: center; flex-wrap: wrap; position: relative; z-index: 1; }
  .modal-footer .secondary { color: var(--text-mute); font-size: 12px; }
  .btn-primary {
  ...
  @media (max-width: 800px) {
    ...
    #mobile-fab {
      display: flex; position: fixed; top: 14px; right: 14px;
      width: 44px; height: 44px; border-radius: 50%;
      background: rgba(8, 8, 14, 0.86); color: var(--catB);
      border: 1px solid var(--card-border-strong);
...
<div class="modal-bg" id="modal-bg" role="dialog" aria-modal="true" aria-labelledby="modal-title">
  <div class="modal" tabindex="-1">
    <div class="modal-header">
      ...
      <p class="sub">Hi. So finding a decent veggie restaurant in Hong Kong shouldn't be this annoying — OpenRice is full of paid 5★ reviews, HappyCow recycles the same 10 expat spots, Google reviews split bimodal between 5s and 1s with nothing in between. So I roped in <a href="https://anthropic.com" target="_blank" rel="noopener noreferrer" style="color:var(--catB);">Claude</a> and built this.</p>
    </div>
    <div class="modal-body">
      <div class="stat-row">
        ...
        <div class="stat-pill"><span class="num">4</span><span class="lbl">categories</span></div>
      </div>

      <h3>what's in it</h3>
      <ul>
        <li><strong style="color:var(--catA)">Chinese 齋</strong> — Buddhist veg halls, mock-meat, traditional dim sum.</li>
        <li><strong style="color:var(--catB)">Modern plant-based</strong> — the newer-wave vegan/vegetarian places.</li>
        <li><strong style="color:var(--catC)">Mainstream with a real veg menu</strong> — non-veg restaurants where the veg side isn't an afterthought.</li>
        <li><strong style="color:var(--catD)">Indian / ME / Med</strong> — pure-veg & naturally veg-heavy cuisines.</li>
      </ul>

      <h3>how the score works</h3>
      <p>Each place gets a <strong>0–100 score</strong> that's actually defensible:</p>
      <ul>
        <li>Bayesian-weighted ratings across Google, OpenRice & HappyCow — so a 4.9★ with 8 reviews can't outrank a 4.5★ with 800.</li>
        <li>Critic bonus: <code>+15</code> ★★★ Michelin, <code>+10</code> ★★, <code>+7</code> ★, <code>+4</code> Bib/Tatler/Time Out.</li>
        <li>Penalties for suspect reviews (those bimodal OpenRice patterns) and dodgy operating status.</li>
      </ul>
      <p>Tiers: <strong style="color:var(--tierS)">S</strong> (85+), <strong style="color:var(--tierA)">A</strong> (75–84), <strong style="color:var(--tierB)">B</strong> (65–74), <strong style="color:var(--tierC)">C</strong> (55–64), <strong style="color:var(--tierD)">D</strong> (&lt;55). Closed places sink to the bottom.</p>

      <h3>how to use it</h3>
      <ul>
        <li>List sorts S → D. Tap anything to fly the map there.</li>
        <li>Slide the score, hit tier/category chips, search by name or dish.</li>
        <li>"Critic-recognised" filters to Michelin/Tatler/Time Out only.</li>
        <li>"Trusted reviews" hides places with iffy review patterns.</li>
        <li>Every popup has a Google Maps button. Phone & website links are tappable.</li>
      </ul>

      <p style="color:var(--text-mute);font-size:12px;margin-top:16px;">Compiled April 2026 with Claude doing the heavy lifting. Restaurant details shift constantly in HK — phone or check Instagram before high-stakes plans. — <a href="https://chitintim.github.io" target="_blank" rel="noopener noreferrer" style="color:var(--text-dim);">Tim</a></p>
    </div>
    <div class="modal-footer">
      <span class="secondary">Tap <span style="display:inline-flex;width:22px;height:22px;border-radius:50%;border:1px solid var(--card-border-strong);align-items:center;justify-content:center;font-size:11px;color:var(--catB);vertical-align:middle;">?</span> anytime</span>
  ...
  <aside id="sidebar">
    <header id="sidebar-header" role="button" aria-label="Toggle results panel">
      ...
      <div class="header-buttons">
        <button class="icon-btn" id="info-btn" title="About / methodology" aria-label="About">?</button>
      </div>
    </header>
...
function closeModal() { modalBg.classList.remove("open"); document.body.style.overflow = ""; try { localStorage.setItem("hkv-seen-welcome-v3", "1"); } catch (_) {} }
...
try { if (!localStorage.getItem("hkv-seen-welcome-v3")) openModal(); } catch (_) { openModal(); }
